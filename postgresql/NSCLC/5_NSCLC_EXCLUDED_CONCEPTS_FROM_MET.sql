...
combined_population_exclusions AS (
  SELECT DISTINCT 
    ...
    (CAST(diags_to_exclude.condition_start_date AS DATE) - CAST(mets.first_measurement_date AS DATE)) as days_to_diag,
    ROW_NUMBER() OVER (PARTITION BY mets.person_id, diags_to_exclude.condition_concept_id ORDER BY ABS((CAST(diags_to_exclude.condition_start_date AS DATE) - CAST(mets.first_measurement_date AS DATE))), (CAST(diags_to_exclude.condition_start_date AS DATE) - CAST(mets.first_measurement_date AS DATE))) as rn  
  FROM mets
