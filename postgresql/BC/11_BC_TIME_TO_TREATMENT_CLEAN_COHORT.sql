...
combined_population_treated AS (
  SELECT DISTINCT 
    mets.person_id, 
    (CAST(min(de.drug_exposure_start_date) AS DATE) - CAST(min(mets.first_measurement_date) AS DATE)) as days_to_treatment  
   FROM mets
...
SELECT 
  ...
  CASE WHEN n < 5 THEN NULL ELSE MIN(CASE
            ...
            END) END AS q5_days_to_treatment,
  CASE WHEN n < 5 THEN NULL ELSE MIN(CASE
            WHEN order_nr < .10 * n
                THEN 9999
            ELSE days_to_treatment
            END) END AS q10_days_to_treatment,
  CASE WHEN n < 5 THEN NULL ELSE MIN(CASE
            WHEN order_nr < .25 * n
                THEN 9999
            ELSE days_to_treatment
            END) END AS q25_days_to_treatment,
  CASE WHEN n < 5 THEN NULL ELSE MIN(CASE
            WHEN order_nr < .50 * n
                THEN 9999
            ELSE days_to_treatment
  ...
  CASE WHEN n < 5 THEN NULL ELSE MIN(CASE
            WHEN order_nr < .75 * n
                THEN 9999
            ELSE days_to_treatment
            END) END AS q75_days_to_treatment,
  CASE WHEN n < 5 THEN NULL ELSE MIN(CASE
            WHEN order_nr < .9 * n
                THEN 9999
            ELSE days_to_treatment
            END) END AS q90_days_to_treatment,
  CASE WHEN n < 5 THEN NULL ELSE MIN(CASE
            WHEN order_nr < .95 * n
                THEN 9999
            ELSE days_to_treatment
            END) END AS q95_days_to_treatment,
